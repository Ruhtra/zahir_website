...
class ErrorsFunctions {
  ...
  insert(path, msg) {
    ...
    if (path[0] == "telephones") {
      element = this.form.querySelectorAll(`#telephones .${path[1]}`)[path[2]];
    } else if (path[0] == "category" && path[1] == "type") {
      //   var nelement = this.form.querySelector(`#${path.join(" #")}`);
    } else {
      element = this.form.querySelector(`#${path.join(" #")}`);
    }
    ...
    if (element) {
      ...
      console.error("Erro ao exibir erros no formulário");
    }
  }
}
...
class TelephoneFunctions {
  constructor(baseTelephone) {
    ...
    // Build buttons
    this.baseTelephone
      .querySelector('.insert-cell [value="whatsapp"]')
      .addEventListener("click", () => this.add.whatsapp());
    this.baseTelephone
      .querySelector('.insert-cell [value="telephone"]')
      .addEventListener("click", () => this.add.telephone());
  }
  addMain(type, value) {
    if (!type) throw new Error("The type parameter must be passed");
    type = type.trim().toLowerCase();
    if (!["telephone", "whatsapp"].includes(type))
      throw new Error('Type passed must be "telephone" or "whatsapp"');

    this.baseTelephone.insertAdjacentHTML("beforeend", Filter.telephone(type));
    let element = [...this.baseTelephone.querySelectorAll("." + type)].slice(
      -1
    )[0];
    ...
    element.addEventListener("keyup", () =>
      this.mask(element.querySelector('input[type="text"]'))
    );
    element
      .querySelector('input[name="delete"]')
      .addEventListener("click", () => this.delete(element));
  }
  add = {
...
class Functions {
  constructor(form) {
    ...
    this.telephoneFunctions = new TelephoneFunctions(
      form.querySelector("#telephones")
    );
  ...
  get = {
    ...
    category: {
      type: () => [
        ...this.form.querySelectorAll('#category #type input[type="checkbox"]'),
      ],
      categories: () => [
        ...this.form.querySelectorAll(
          '#category #categories .item input[type="checkbox"]'
        ),
      ],
      newCategories: () => [
        ...this.form.querySelectorAll(
          '#category #newCategories .item input[type="checkbox"]'
        ),
      ],
      newCategorieText: () =>
        this.form.querySelector("#category #newCategories #new input"),
    },
    informations: () => this.form.querySelector("#informations textarea"),
    telephones: {
      telephone: () => [
        ...this.form.querySelectorAll(
          'div#telephones .telephone input[type="text"]'
        ),
      ],
      whatsapp: () => [
        ...this.form.querySelectorAll(
          'div#telephones .whatsapp input[type="text"]'
        ),
      ],
    },
    local: {
    ...
    promotion: {
      ...
      description: () =>
        this.form.querySelector("#promotion #description textarea"),
    },
  };
  ...
  getDataInsert() {
    ...
    let data = {
      ...
      telephones: {
        telephone: this.get.telephones
          .telephone()
          .map((e) => "+55" + e.value.replace(/\D/g, "")),
        whatsapp: this.get.telephones
          .whatsapp()
          .map((e) => "+55" + e.value.replace(/\D/g, "")),
      },
      local: {
    ...
    data.category["type"] = [
      ...this.form.querySelectorAll("#category #type input:checked"),
    ].map((e) => e.getAttribute("name"));
    data.category["categories"] = [
      ...this.form.querySelectorAll("#category #categories input:checked"),
    ].map((e) => e.getAttribute("name"));
    data.category["newCategories"] = [
      ...this.form.querySelectorAll("#category #newCategories input:checked"),
    ].map((e) => e.getAttribute("name"));
  ...
  set(data) {
    ...
    if (data.category.type) {
      data.category.type.forEach((e) => {
        this.form.querySelector(
          `#category #type label[name="${e}"] input`
        ).checked = true;
      });
    }
    if (data.category.categories) {
      data.category.categories.forEach((e) => {
        this.form.querySelector(
          `#category #categories div[name="${e}"] input`
        ).checked = true;
      });
    }
    ...
    if (data.local) {
      ...
      if (data.local.complement)
        this.get.local.complement().value = data.local.complement;
    }

    if (data.movie) this.get.movie().value = data.movie;
    // Promotion
    this.get.promotion.active().checked = data.promotion.active;
    if (data.promotion.title)
      this.get.promotion.title().value = data.promotion.title;
    if (data.promotion.description)
      this.get.promotion.description().value = data.promotion.description;
  }
  async new(data) {
  ...
  clear() {
    ...
    this.get.category.type().forEach((e) => {
      e.checked = false;
    });
...
class Form {
  constructor(form) {
    ...
    btn.insert.addEventListener("click", (evt) => {
      ...
      this.db.profile.insert(data, file).finally(() => {
    ...
    btn.update.addEventListener("click", (evt) => {
      ...
      this.db.profile.update(data, file).finally(() => {
